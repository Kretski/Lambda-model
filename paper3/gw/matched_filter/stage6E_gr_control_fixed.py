...
def generate_taylorf2_template(waveform, freqs):
    mask = (
        (freqs >= F_LOW) &
        (freqs <= F_HIGH)
    )
    ...
    h = waveform.taylorf2_leading_order(
        f_band,
        M1,
        M2,
        tc=0.0,
...
def main():
    ...
    mask_h, f_h, template = generate_taylorf2_template(
        waveform,
        freqs
    )
    ...
    if len(template) != len(f_band):
        raise RuntimeError(
            "Template/data frequency masks do not agree."
        )

    # --------------------------------------------------------
    # 6. Template phase alignment
    # --------------------------------------------------------

    # TaylorF2 generator already receives tc=0.
    # For this control we use the extracted segment itself
    # and test its GR compatibility.

    h_fd = np.zeros_like(data_fd, dtype=complex)
    h_fd[mask_h] = template
