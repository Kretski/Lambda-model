...
## Repository structure
    ...
    │   │   │   ├── stage5J_branch_continuation.py        # Automated branch continuation + pre-registered fold/breakdown classifier
    │   │   │   ├── stage5K_1_observable_extraction.py    # f_R(m), tau(m), Q(m) from the validated branch only
    │   │   │   ├── stage5K_3_ringdown_injection_recovery.py  # Noiseless synthetic-ringdown recovery sanity check
    │   │   │   ├── stage5K_5_noise_robustness.py         # SNR-sweep (30/20/10/7) recovery bias/variance/success-rate test
    │   │   │   └── STAGE5J_5K_STATUS.md     # Frozen status writeup — "candidate critical QNM branch termination"
...
## Quick start
    ...
    python stage5J_branch_continuation.py          # branch continuation + classification
    python stage5K_1_observable_extraction.py      # f_R(m), tau(m), Q(m) from the validated branch
    python stage5K_3_ringdown_injection_recovery.py  # noiseless recovery sanity check
    python stage5K_5_noise_robustness.py           # SNR-sweep recovery robustness
