...
3. **If you work with BEC:** the mapping $\Lambda=\xi^2/4$ is physicallyestablished (exact match to the Bogoliubov dispersion relation, not ananalogy). Plug in your healing length and sound speed and test itagainst your own Bragg-spectroscopy data directly.
...
| Giant-vortex analog QNM branch-tracking & response function (Stage 5I–5K) | ⚠️ **Candidate critical QNM branch termination — not established.** Numerical branch-tracking of the Λ-model's q=2 quasinormal-mode branch for the superfluid-helium giant-vortex analog system (Smaniotto et al. experimental target), validated against a two-stage NM→hybr solver (acceptance |Res|<1e-8). Two independently-validated candidate q=2 roots exist at m=−11..−14 ("Root A", the original discovery-grid assignment; "Root B", found via a later coverage/sensitivity check, matching the experimental frequency far more closely and smoothly continuous with the rest of the q=2 family — adopted as the primary working branch). **Root B** tracks cleanly from m=−14 to m≈−14.9055 before an abrupt breakdown with no fold signature (Jacobian singular value does not collapse), and an independent 41-seed search at m=−15 finds 0 validated poles; **Root A** (retained as a secondary/diagnostic branch) shows the same qualitative pattern, breaking down earlier at m≈−14.7725 (51-seed search, 0 validated). Observables (f_R, τ, Q) extracted only for each branch's validated range and shown recoverable from a synthetic ringdown under noise (SNR 30→7, graceful degradation, no persistent bias, both roots). **Next step (deliberately deferred, not yet done): compare against real GW ringdown parameter estimation before any physical-effect claim.** Full writeup: `paper3/gw/matched_filter/STAGE5J_5K_STATUS.md`. |
...
## Repository structure
    ...
    │   │   ├── _archive/qnm_branch_work/    # Giant-vortex analog QNM branch-tracking (Stage 5I-5K)
    ...
    │   │   ├── STAGE5J_5K_STATUS.md         # Frozen status writeup -- Root A vs Root B, "candidate critical QNM branch termination"
...
## Quick start
    ...
    # Giant-vortex analog QNM branch-tracking (Stage 5I-5K) -- candidate
    # critical branch termination, NOT an established physical result;
    # see the status table entry and STAGE5J_5K_STATUS.md before citing.
...
This is a **falsifiable framework**, not a confirmed law of nature. Asingle value of $\Lambda$ that is *simultaneously* consistent withFermi-LAT, EHT, GW, and BEC data would be evidence of a shared underlyingmechanism. Inconsistent values would falsify the universal-$\Lambda$hypothesis while leaving each domain-specific dispersion law intact.Note that this universality is a hypothesis under test, not anassumption of the framework: the photon-sector and graviton-sectorbounds quoted above are independent constraints and are not currentlyrequired to agree.
